...
interface IDisputeResolver {
    ...
    /// @notice Raise a dispute for a session (called by hub)
    /// @param caller Address of the caller (student or tutor)
    /// @param sessionId ID of the session to dispute
    /// @param reason Reason for the dispute
    /// @param evidence IPFS hash or evidence reference
    /// @return disputeId The ID of the created dispute
    ...
    function resolveDispute(uint256 disputeId, DisputeLib.DisputeOutcome outcome) external;
