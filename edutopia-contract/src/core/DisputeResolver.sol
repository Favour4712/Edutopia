...
contract DisputeResolver is IDisputeResolver, Ownable {
    ...
    constructor(address _sessionEscrow, uint256 _disputeWindow) Ownable(msg.sender) {
    ...
    {
        return _raiseDispute(msg.sender, sessionId, reason, evidence);
    }

    /// @inheritdoc IDisputeResolver
    function raiseDisputeFor(address caller, uint256 sessionId, string calldata reason, string calldata evidence)
        external
        override
        returns (uint256 disputeId)
    {
        return _raiseDispute(caller, sessionId, reason, evidence);
    }

    /// @notice Internal function to raise a dispute
    function _raiseDispute(address caller, uint256 sessionId, string calldata reason, string calldata evidence)
        internal
        returns (uint256 disputeId)
    {
        ...
        emit Events.DisputeRaised(disputeId, sessionId, caller, reason);

        return disputeId;
    }

    /// @inheritdoc IDisputeResolver
    function resolveDispute(uint256 disputeId, DisputeLib.DisputeOutcome outcome) external override onlyArbiter {
        ...
        emit Events.DisputeResolved(disputeId, dispute.sessionId, msg.sender, refundToStudent, refundAmount);
    }
